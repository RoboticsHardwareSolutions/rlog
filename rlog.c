...
static int log_level =
    RLOG_ERROR_PRINT | RLOG_WARNING_PRINT | RLOG_INFO_PRINT | RLOG_DEBUG_PRINT | RLOG_DATA_AND_TIME_PRINT;
...
#        define LOCALTIME_BUF_SIZE (30) /* Buffer size for holding strftime timestamp */
    ...
    strftime(localtime_buf,
             LOCALTIME_BUF_SIZE,
             format,
             localtime(
                 &ts.tv_sec)); /* note: localtime returns pointer to internal tm object which is managed externally */
